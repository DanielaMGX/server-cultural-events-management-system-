...
async def read(id: int):
    mode = await service_contractual_mode.get_by_id(id)
    if mode is None:
...
async def update(id: int, mode: UpdateContractualMode):
    updated = await service_contractual_mode.update(id, obj_in=mode)
    if not updated:
        raise HTTPException(status_code=404, detail="Mode not found")

@router.delete("/{id}", status_code=204)
async def delete(id: int):
    deleted = await service_contractual_mode.delete(id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Mode not found")
